  ...
  return (
    <div className={`overflow-hidden transition-colors duration-300 fixed w-full bg-black top-0 left-0 z-10 ${isPopupOpen ? 'bg-opacity-80 h-full' : 'bg-opacity-0 h-0'}`}>
        <div className='p-4 w-full h-full overflow-auto flex items-center justify-center'>
            <div className={`relative p-3 bg-white w-full max-w-2xl transition-all duration-300 ease-in-out ${isPopupOpen ? 'translate-y-0 opacity-1' : 'translate-y-5 opacity-0'}`}>
